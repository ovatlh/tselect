var tselect=tselect||(function () {
  ...
  function fnToggleAll(id) {
    try {
      ...
      if(_SELECT_DATA[id].optionSelectedList.length != _SELECT_DATA[id].optionList.length) {
  ...
  function _refresh_option_list_dom(id) {
    try {
      ...
      const _option_list_value = _SELECT_DATA[id].optionList.map((item) => item.value);
      const result_is_same_values = _compare_2_arrays(_SELECT_DATA[id].optionSelectedList, _option_list_value);
